...
# -*- coding: utf-8 -*-
"""
EUDAMED Platform Actors Raw acquisition pipeline.
...
- Current domain implemented: Actors (economic operators) endpoint.
...
Release naming
--------------
Latest tag: eudamed-platform-actors-raw-latest
Latest assets:
  eudamed_platform_actors_raw_latest.duckdb
  eudamed_platform_actors_raw_latest_csv.zip
  eudamed_platform_actors_raw_latest.metadata.json
  RELEASE_NOTES_EUDAMED_PLATFORM_ACTORS_RAW.md

Dated tag: eudamed-platform-actors-raw-YYYYMMDD_HHMMSS
Dated assets:
  eudamed_platform_actors_raw_YYYYMMDD_HHMMSS.duckdb
  eudamed_platform_actors_raw_YYYYMMDD_HHMMSS_csv.zip
  eudamed_platform_actors_raw_YYYYMMDD_HHMMSS.metadata.json
  RELEASE_NOTES_EUDAMED_PLATFORM_ACTORS_RAW_YYYYMMDD_HHMMSS.md
"""
...
def flatten_actor(row: Dict[str, Any], page_number: int, extract_date: str, extract_ts: str) -> Dict[str, Any]:
    actor_type = row.get("actorType") or {}
    actor_status = row.get("actorStatus") or {}
    return {
        ...
        "registration_ulid": row.get("registrationUlid"),
        "registration_ulid_timestamp": decode_ulid(row.get("registrationUlid")),
        "srn": row.get("srn"),
        "eudamed_identifier": row.get("eudamedIdentifier"),
        "name": row.get("name"),
        "names_json": jdump(row.get("names")),
        ...
        "actor_status_from_date": row.get("actorStatusFromDate"),
        "role_name": row.get("roleName"),
        "country_iso2_code": row.get("countryIso2Code"),
        "country_name": row.get("countryName"),
        "country_type": row.get("countryType"),
        "date_of_registration": row.get("dateOfRegistration"),
        "electronic_mail": row.get("electronicMail"),
        "telephone": row.get("telephone"),
        "geographical_address": row.get("geographicalAddress"),
        "building_number": row.get("buildingNumber"),
        "street_name": row.get("streetName"),
        "postbox": row.get("postbox"),
        "address_complement": row.get("addressComplement"),
        "postal_zone": row.get("postalZone"),
        "city_name": row.get("cityName"),
        "abbreviated_name": row.get("abbreviatedName"),
        "abbreviated_names_json": jdump(row.get("abbreviatedNames")),
        "latest_version": row.get("latestVersion"),
        "version_number": row.get("versionNumber"),
        "associated_to_user": row.get("associatedToUser"),
        "legislation_links_json": jdump(row.get("legislationLinks")),
        "selectable": row.get("selectable"),
        "actor_validated": row.get("actorValidated"),
        "nb_status": row.get("nbStatus"),
        "raw_json": jdump(row),
        "source_endpoint": "eos",
        "source_page": page_number,
    }

ACTOR_COLUMNS = list(flatten_actor({}, 0, "", "").keys())
...
class EudamedClient:
    def __init__(self, base_url: str, language: str, page_size: int, timeout: int, retries: int, backoff: float, max_rps: float):
        ...
        self.session.headers.update({
            "User-Agent": f"EUDAHUB-Intelligence Platform Actors Raw {PIPELINE_VERSION}",
    ...
    def fetch_page(self, page: int, include_historical: bool = False) -> Tuple[int, int, Optional[Dict[str, Any]], Optional[str], float, Optional[str]]:
        url = f"{self.base_url}{ACTORS_ENDPOINT}"
...
def existing_actor_table(con: duckdb.DuckDBPyConnection) -> Optional[str]:
...
def find_existing_db(inputs_dir: Path) -> Optional[Path]:
    candidates = [
        inputs_dir / "eudamed_platform_actors_raw_latest.duckdb",
        inputs_dir / "eudamed_platform_actors_raw_latest.duckdb.zip",
    ]
    for p in candidates:
...
def read_existing_rows(db_path: Path) -> List[Dict[str, Any]]:
    ...
    try:
        table = existing_actor_table(con)
        if not table:
            return []
        ...
        mapping = {
            "uuid": ["uuid"],
            "ulid": ["ulid"],
            "registration_ulid": ["registrationUlid", "registration_ulid"],
            "srn": ["srn"],
            "eudamed_identifier": ["eudamedIdentifier", "eudamed_identifier"],
            "name": ["name"],
            "actor_type_code": ["actor_type_code"],
            "actor_type_srn_code": ["actor_type_srn_code"],
            "actor_type_category": ["actor_type_category"],
            "actor_status_code": ["actor_status_code"],
            "actor_status_from_date": ["actorStatusFromDate", "actor_status_from_date"],
            "role_name": ["roleName", "role_name"],
            "country_iso2_code": ["countryIso2Code", "country_iso2_code"],
            "country_name": ["countryName", "country_name"],
            "country_type": ["countryType", "country_type"],
            "date_of_registration": ["dateOfRegistration", "date_of_registration"],
            "electronic_mail": ["electronicMail", "electronic_mail"],
            "telephone": ["telephone"],
            "geographical_address": ["geographicalAddress", "geographical_address"],
            "postal_zone": ["postalZone", "postal_zone"],
            "city_name": ["cityName", "city_name"],
            "latest_version": ["latestVersion", "latest_version"],
            "version_number": ["versionNumber", "version_number"],
            "associated_to_user": ["associatedToUser", "associated_to_user"],
            "selectable": ["selectable"],
            "actor_validated": ["actorValidated", "actor_validated"],
            "nb_status": ["nbStatus", "nb_status"],
        }
        for r in records:
            d = {c: None for c in ACTOR_COLUMNS}
            for c in ACTOR_COLUMNS:
                if c in r:
                    d[c] = r.get(c)
            ...
            d["registration_ulid_timestamp"] = d.get("registration_ulid_timestamp") or decode_ulid(d.get("registration_ulid"))
            d["source_endpoint"] = d.get("source_endpoint") or "eos"
            out.append(d)
        return out
    finally:
        con.close()
...
def parse_page(data: Dict[str, Any], page: int, extract_date: str, extract_ts: str) -> List[Dict[str, Any]]:
    return [flatten_actor(item, page, extract_date, extract_ts) for item in data.get("content", [])]
...
def read_previous_metadata(inputs_dir: Path) -> Dict[str, Any]:
    path = inputs_dir / "eudamed_platform_actors_raw_latest.metadata.json"
    if not path.exists():
        return {}
    try:
...
def write_duckdb(out_db: Path, rows: List[Dict[str, Any]], initial_page: Dict[str, Any], page_audit: List[Dict[str, Any]], request_log: List[Dict[str, Any]], metadata: Dict[str, Any]) -> None:
    ...
    try:
        con.register("actors_raw_df", pd.DataFrame(rows, columns=ACTOR_COLUMNS))
        con.execute("CREATE TABLE actors_raw AS SELECT * FROM actors_raw_df")
        ...
        for col in ACTOR_COLUMNS:
            field_rows.append({"table_name": "actors_raw", "field_name": col, "non_null_rows": sum(1 for r in rows if r.get(col) is not None), "total_rows": len(rows)})
...
def write_csv_zip(zip_path: Path, rows: List[Dict[str, Any]]) -> None:
    ...
    if tmp.exists():
        shutil.rmtree(tmp)
    ensure_dir(tmp)
    csv_path = tmp / "actors_raw.csv"
    with csv_path.open("w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=ACTOR_COLUMNS, extrasaction="ignore")
        writer.writeheader()
        for r in rows:
            writer.writerow({c: r.get(c) for c in ACTOR_COLUMNS})
    with zipfile.ZipFile(zip_path, "w", compression=zipfile.ZIP_DEFLATED, compresslevel=6) as zf:
        zf.write(csv_path, "actors_raw.csv")
    shutil.rmtree(tmp)
...
def write_release_notes(path: Path, metadata: Dict[str, Any]) -> None:
    title = "# EUDAMED Platform Actors Raw"
    lines = [
        title,
        "",
        ...
        f"Body include historical: `{metadata.get('body_include_historical')}`",
        "",
        "## Snapshot",
        "",
        ...
        f"- Min registration ULID: `{metadata.get('min_registration_ulid')}` → `{metadata.get('min_registration_ulid_timestamp')}`",
        f"- Max registration ULID: `{metadata.get('max_registration_ulid')}` → `{metadata.get('max_registration_ulid_timestamp')}`",
        "",
        "## Audit",
        "",
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    ...
    p.add_argument("--out-dir", default="dist/eudamed_platform_actors_raw")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    log(f"=== EUDAMED Platform Actors Raw {PIPELINE_VERSION} acquisition started ===")
    log(f"requested_mode={args.mode} out_dir={out_dir} page_size={args.page_size}")
    log("scope=raw_fetch_only cdc=0 canonical=0 dk_subset=0 domain=actors")
    ...
    bootstrap = False
    if not existing_rows and args.mode == "full":
        bootstrap = True
        log("Full requested and no previous DB exists. Creating BOOTSTRAP full crawl.")
    elif args.mode in {"incremental", "resume_full"} and not existing_rows:
        effective_mode = "full"
        bootstrap = True
    ...
    if status != 200 or not initial_data:
        ...
        write_json(out_dir / f"eudamed_platform_actors_raw_{rel_ts}.metadata.json", metadata)
        write_json(out_dir / "eudamed_platform_actors_raw_latest.metadata.json", metadata)
        write_release_notes(out_dir / "RELEASE_NOTES_EUDAMED_PLATFORM_ACTORS_RAW.md", metadata)
    ...
    if body_include_historical and not (audit or {}).get("current_initial_page"):
        log("=== Current actors initial page probe page=0 includeHistoricalVersion=omitted ===")
        _, current_status, current_data, current_error, current_elapsed_ms, current_retry_after = client.fetch_page(0, include_historical=False)
        current_initial_probe = initial_page_record(current_data, current_status, current_error, current_elapsed_ms, current_retry_after, args.page_size)
        log(json.dumps({"current_initial_page_probe": current_initial_probe}, ensure_ascii=False))
    ...
    # when page 0 responded OK and the frontier check found no new actor records.
    ...
    distinct_eudamed_identifier_count = len({r.get("eudamed_identifier") for r in merged_rows if r.get("eudamed_identifier")})
    ...
    latest_version_false_count = sum(1 for r in merged_rows if str(r.get("latest_version")).lower() == "false" or r.get("latest_version") is False)
    min_reg_u = min_ulid(r.get("registration_ulid") for r in merged_rows)
    max_reg_u = max_ulid(r.get("registration_ulid") for r in merged_rows)
    ...
    metadata = {
        ...
        "current_initial_page": current_initial_page,
        "distinct_uuid_count": distinct_uuid_count,
        "distinct_ulid_count": distinct_ulid_count,
        "distinct_srn_count": distinct_srn_count,
        "distinct_eudamed_identifier_count": distinct_eudamed_identifier_count,
        ...
        "min_registration_ulid": min_reg_u,
        "min_registration_ulid_timestamp": decode_ulid(min_reg_u),
        "max_registration_ulid": max_reg_u,
        "max_registration_ulid_timestamp": decode_ulid(max_reg_u),
    ...
    latest_db = out_dir / "eudamed_platform_actors_raw_latest.duckdb"
    latest_csv = out_dir / "eudamed_platform_actors_raw_latest_csv.zip"
    latest_meta = out_dir / "eudamed_platform_actors_raw_latest.metadata.json"
    latest_notes = out_dir / "RELEASE_NOTES_EUDAMED_PLATFORM_ACTORS_RAW.md"
    dated_db = out_dir / f"eudamed_platform_actors_raw_{rel_ts}.duckdb"
    dated_csv = out_dir / f"eudamed_platform_actors_raw_{rel_ts}_csv.zip"
    dated_meta = out_dir / f"eudamed_platform_actors_raw_{rel_ts}.metadata.json"
    dated_notes = out_dir / f"RELEASE_NOTES_EUDAMED_PLATFORM_ACTORS_RAW_{rel_ts}.md"
    ...
    log("=== EUDAMED Platform Actors Raw complete ===")
    log(json.dumps({
        "run_status": run_status,
        "requested_mode": args.mode,
        "effective_mode": effective_mode,
